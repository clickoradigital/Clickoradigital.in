...
const CircuitLine = ({ path, endX, endY, duration, delay }: { path: string; endX: number; endY: number; duration: number; delay: number }) => (
  <g>
    ...
    <motion.path
      d={path}
      fill="none"
      stroke="#e8f4dc" // Lightest sage for maximum glow on dark background
      strokeWidth="2.5"
      strokeLinecap="round"
      initial={{ pathLength: 0.05, pathOffset: 0, opacity: 0 }}
      animate={{ 
        pathOffset: [0, 1],
        opacity: [0, 1, 1, 0] // Fades in, travels bright, fades out
      }}
      transition={{
        duration: duration,
        delay: delay,
      ...
      style={{ filter: "drop-shadow(0px 0px 8px rgba(232,244,220,0.8))" }}
    />
  </g>
);
...
const ProcessReactor: React.FC = () => {
  ...
  return (
    // Deep dark zinc/sage background to contrast the white sections above
    <section className="relative bg-[#2A311F] py-32 lg:py-40 overflow-hidden rounded-[2rem] lg:rounded-[4rem] mx-2 lg:mx-4 my-10">
      
      {/* --- AMBIENT BACKGROUND GLOWS --- */}
      <motion.div 
        animate={{ scale: [1, 1.2, 1], opacity: [0.05, 0.1, 0.05] }}
        transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
        className="absolute top-1/4 left-0 w-96 h-96 bg-[#b6c99c] rounded-full blur-[120px] pointer-events-none"
      />
      <motion.div 
        animate={{ scale: [1, 1.5, 1], opacity: [0.03, 0.08, 0.03] }}
        transition={{ duration: 10, repeat: Infinity, ease: "easeInOut", delay: 2 }}
        className="absolute bottom-1/4 right-0 w-[30rem] h-[30rem] bg-[#cfe1bb] rounded-full blur-[150px] pointer-events-none"
      />
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 relative z-20">
        ...
        <div className="mb-20 lg:mb-32 text-left lg:text-center">
          <motion.div 
            ...
            className="inline-flex items-center gap-4 mb-6 px-4 py-2 rounded-full border border-white/10 bg-black/10 backdrop-blur-sm"
          >
            <div className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#cfe1bb] opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-[#e8f4dc]"></span>
            </div>
            <span className="font-lato  text-[10px] uppercase tracking-[0.6em] text-[#e8f4dc] opacity-80 font-bold">System Sequence</span>
          </motion.div>
          ...
          <div className="overflow-hidden">
            <motion.h2 
              ...
              className="font-glyseric  text-6xl md:text-[9rem] text-[#e8f4dc] leading-none tracking-tighter"
            >
        ...
        <div ref={trackRef} className="relative py-10 lg:py-20">
          ...
          <div className="absolute left-[2rem] lg:left-1/2 top-0 bottom-0 w-[1px] bg-white/5 -translate-x-1/2 z-0">
            <motion.div 
              style={{ scaleY }}
              className="absolute top-0 w-[3px] bg-[#e8f4dc] blur-sm origin-top h-full -translate-x-[1px]"
            />
            <motion.div 
              style={{ scaleY }}
              ...
              >
                ...
                >
                  ...
                  <motion.span 
                    ...
                    className="font-glyseric  text-8xl lg:text-[14rem] text-[#e8f4dc] opacity-[0.03] leading-none absolute -top-10 lg:-top-24 select-none pointer-events-none group-hover:opacity-[0.06] transition-opacity duration-700"
                  >
                    {step.no}
                  </motion.span>

                  <div className="relative pt-4 z-10">
                    <motion.span variants={itemVariants} className="font-lato  text-[10px] uppercase tracking-widest text-[#b6c99c] font-bold mb-3 block ">
                      {step.label}
                    </motion.span>
                    <motion.h3 variants={itemVariants} className="font-glyseric  text-5xl lg:text-7xl text-[#e8f4dc] mb-6 tracking-tighter group-hover:italic group-hover:text-white transition-all duration-500">
                      {step.title}
                    </motion.h3>
                    <motion.p variants={itemVariants} className="font-lato  text-[#b6c99c] text-base lg:text-lg leading-relaxed max-w-sm font-medium group-hover:text-[#e8f4dc] transition-colors duration-500">
                      {step.desc}
                    </motion.p>
                  </div>
                </motion.div>
        ...
        >
          <div className="relative group cursor-pointer">
            <motion.div 
              ...
              className="absolute inset-[-20%] rounded-full bg-gradient-to-r from-transparent via-[#b6c99c]/30 to-transparent blur-[40px] opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" 
            />
            
            <motion.button 
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="relative z-10 w-44 h-44 lg:w-64 lg:h-64 rounded-full bg-[#2A311F] flex flex-col items-center justify-center gap-2 group border border-white/10 transition-all duration-500 overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.5)] hover:border-[#b6c99c]"
            >
                ...
                <span className="relative z-10 font-lato  text-[10px] font-black uppercase tracking-widest text-[#b6c99c] group-hover:text-[#728156] transition-colors duration-500">
                  Initiate
                </span>
                
                <span className="relative z-10 font-glyseric  text-3xl lg:text-5xl text-[#e8f4dc] group-hover:text-[#2A311F] transition-colors duration-500">
                  Reaction
                </span>

                <span className="relative z-10 mt-2 text-[#88976c] group-hover:text-[#728156] transition-colors duration-500">
                  ✦
                </span>
            </motion.button>
          </div>
          ...
          <div className="mt-24 flex flex-col items-center gap-4">
            <motion.div 
              ...
              className="w-[1px] bg-gradient-to-b from-[#b6c99c] to-transparent" 
            />
            <span className="font-lato  text-[9px] uppercase tracking-[0.6em] text-[#88976c] font-bold"> 
              Sequence Complete 
            </span>
          </div>
        </motion.div>
